...
const testUser = {
  name: 'Core Functionality Tester',
  email: `corefunctionality${timestamp}@example.com`,
  password: 'TestPassword123!',
  employeeNumber: `EMP-CORE-${timestamp}`,
  department: 'Data Analytics',
  username: `corefunctionality${timestamp}`
};

const testBrand = {
  name: `Core Functionality Brand ${timestamp}`,
  description: 'Brand for core functionality testing',
  website: 'https://corefunctionality.com',
  industry: 'Technology'
};
...
const testCoreFunctionality = async () => {
  ...
  console.log('Testing all core APIs and functionalities');
  console.log('='.repeat(50));
  ...
  try {
    ...
    const projectResponse = await makeRequest('POST', `/brands/${brandId}/projects`, {
      title: 'Core Functionality Project',
      description: 'Project for core functionality testing',
      status: 'Active',
      priority: 'Medium',
      department: 'Data Analytics',
      startDate: new Date(),
    ...
    const updateProjectResponse = await makeRequest('PUT', `/brands/${brandId}/projects/${projectId}`, {
      description: 'Updated project description for core functionality testing'
    });
    ...
    // 11. Create Task
    console.log('11. Testing task creation...');
    ...
    const taskResponse = await makeRequest('POST', `/brands/${brandId}/tasks`, {
      ...
      description: 'Task for core functionality testing',
      status: 'Yet to Start',
      priority: 'Medium',
      projectId: projectId,
      assignedTo: userId,
    ...
    const updateTaskResponse = await makeRequest('PUT', `/brands/${brandId}/tasks/${taskId}`, {
      description: 'Updated task description for core functionality testing'
    });
    ...
    // 15. Create Comment
    console.log('15. Testing comment creation...');
    ...
    const mentionResponse = await makeRequest('GET', `/brands/${brandId}/mention-suggestions?query=Core`);
    ...
    console.log('\n🎉 SYSTEM STATUS:');
    if (testResults.failed === 0) {
      console.log('✅ ALL CORE FUNCTIONALITY WORKING - READY FOR FRONTEND INTEGRATION!');
    ...
    console.log('✅ Project Management (Create, Get, Update, Delete)');
    console.log('✅ Task Management (Create, Get, Update, Delete)');
    console.log('✅ Advanced Comment System (Create, Get, Reactions, Replies)');
    console.log('✅ Activity & Notifications (Activities, Preferences)');
    console.log('✅ Analytics (Project, Task, Brand, Comment Statistics)');
    ...
    console.error('❌ CRITICAL ERROR in core functionality test:', error.message);
    return false;
  }
};
