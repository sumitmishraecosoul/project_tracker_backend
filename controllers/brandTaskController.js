...
/**
 * Generate HTML content for task email
 * @param {string} subject - Email subject
 * @param {string} message - Email message
 * @param {Array} tasks - Array of task objects
 * @param {string} regards - Regards text
 * @returns {string} HTML content
 */
const generateTaskEmailHtml = (subject, message, tasks, regards) => {
  const taskRows = tasks.map(task => `
    <tr>
      <td style="padding: 10px; border: 1px solid #ddd;">${task.id || 'N/A'}</td>
      <td style="padding: 10px; border: 1px solid #ddd;">${task.task || 'N/A'}</td>
      <td style="padding: 10px; border: 1px solid #ddd;">${task.status || 'N/A'}</td>
      <td style="padding: 10px; border: 1px solid #ddd;">${task.priority || 'N/A'}</td>
      <td style="padding: 10px; border: 1px solid #ddd;">${task.assignedTo?.name || 'Unassigned'}</td>
      <td style="padding: 10px; border: 1px solid #ddd;">${task.eta ? new Date(task.eta).toLocaleDateString() : 'N/A'}</td>
    </tr>
  `).join('');
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .tasks-table th { background: #007bff; color: white; padding: 12px; text-align: left; border: 1px solid #ddd; }
        .tasks-table td { padding: 10px; border: 1px solid #ddd; }
        .regards { margin-top: 30px; padding-top: 20px; border-top: 1px solid #e9ecef; }
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          ${regards ? `<div class="regards"><p><strong>Regards,</strong><br>${regards}</p></div>` : ''}
        </div>

        <div class="footer">
          <p>This email was generated by Project Tracker. Please do not reply to this email.</p>
...
const sendTasksEmail = async (req, res) => {
  try {
    const { to, subject, message, tasks, regards } = req.body;

    // Validate required fields
    if (!to || !subject || !message || !tasks || !Array.isArray(tasks)) {
      return res.status(400).json({
        success: false,
        error: {
          code: 'VALIDATION_ERROR',
          message: 'to, subject, message, and tasks array are required'
        }
      });
    }
    ...
    // Generate HTML email content with provided tasks
    const emailHtml = generateTaskEmailHtml(subject, message, tasks, regards);

    // Create a mock request/response to use sendMailHandler
    const mockReq = {
      body: {
        to,
        subject,
    ...
    // Send the email using the existing sendMailHandler
    const emailResult = await sendMailHandler(mockReq, mockRes);
    ...
    if (emailResult && emailResult.ok) {
      res.json({
        ...
        message: 'Tasks email sent successfully'
      });
    } else {
      res.status(500).json({
        success: false,
    ...
    res.status(500).json({
      ...
      error: {
        ...
        message: 'Failed to send tasks email',
        details: error.message
      }
    });
  }
