...
async function testAuthentication() {
  ...
  if (loginResult.success) {
    ...
    // Try to register new admin with valid department
...
async function testBrandAPIs() {
  ...
  if (getBrandsResult.success && getBrandsResult.data.brands && getBrandsResult.data.brands.length > 0) {
    ...
    if (createBrandResult.success) {
      ...
      console.log('Error:', createBrandResult.error);
      return false;
    }
  }
}
...
async function testProjectAPIs() {
  ...
  if (getProjectsResult.success && getProjectsResult.data.projects && getProjectsResult.data.projects.length > 0) {
    ...
    // Create new project with valid department and priority
...
async function testTaskAPIs() {
  ...
  // Test 2: Create task with valid status
  ...
  const createTaskResult = await makeRequest('POST', `/brands/${brandId}/tasks`, {
    ...
    estimatedHours: 8,
    status: 'Yet to Start' // Use valid status
  });
...
async function testSubtaskAPIs() {
  ...
  const createSubtaskResult = await makeRequest('POST', `/brands/${brandId}/subtasks`, {
    ...
    estimatedHours: 4,
    status: 'To Do' // Use valid subtask status
  });
