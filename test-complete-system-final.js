...
const testUser = {
  name: 'Complete System Final Tester',
  email: `completesystemfinal${timestamp}@example.com`,
  password: 'TestPassword123!',
  employeeNumber: `EMP-FINAL-${timestamp}`,
  department: 'Data Analytics',
  username: `completesystemfinal${timestamp}`
};

const testBrand = {
  name: `Complete System Final Brand ${timestamp}`,
  description: 'Brand for comprehensive system testing',
  website: 'https://completesystemfinal.com',
  industry: 'Technology'
};

const testProject = {
  title: 'Complete System Final Project',
  description: 'Project for comprehensive system testing',
  status: 'Active',
  priority: 'Medium',
  department: 'Data Analytics',
  startDate: new Date(),
...
const testTask = {
  task: 'Complete System Final Task',
  description: 'Task for comprehensive system testing',
  status: 'Yet to Start',
  priority: 'Medium',
...
const testPhase1 = async () => {
  ...
  try {
    ...
    // 1.3 Get User Profile (using correct endpoint)
    console.log('1.3 Testing get user profile...');
    try {
      const profileResponse = await makeRequest('GET', `/users/${userId}`);
      recordTest('Get User Profile', profileResponse.data.success);
    } catch (error) {
      // Try alternative profile endpoint
      try {
        const profileResponse = await makeRequest('GET', '/users/me');
        recordTest('Get User Profile (me)', profileResponse.data.success);
      } catch (error2) {
        recordTest('Get User Profile', false, 'No working profile endpoint found');
      }
    }
    ...
    try {
      const updateResponse = await makeRequest('PUT', `/users/${userId}`, {
        name: 'Updated Complete System Final Tester'
      });
  ...
  try {
    ...
    try {
      const updateBrandResponse = await makeRequest('PUT', `/brands/${brandId}`, {
        description: 'Updated brand description for comprehensive system testing'
      });
      recordTest('Update Brand', updateBrandResponse.data.success);
    } catch (error) {
      recordTest('Update Brand', false, error.message);
    }
...
const testPhase3 = async () => {
  ...
  try {
    ...
    const updateProjectResponse = await makeRequest('PUT', `/brands/${brandId}/projects/${projectId}`, {
      description: 'Updated project description for comprehensive system testing'
    });
...
const testPhase4 = async () => {
  ...
  try {
    ...
    const updateTaskResponse = await makeRequest('PUT', `/brands/${brandId}/tasks/${taskId}`, {
      description: 'Updated task description for comprehensive system testing'
    });
...
const testPhase5 = async () => {
  ...
  try {
    ...
    const subtaskResponse = await makeRequest('POST', `/brands/${brandId}/tasks/${taskId}/subtasks`, {
      title: 'Complete System Final Subtask',
      description: 'Subtask for comprehensive system testing',
      status: 'Yet to Start',
      priority: 'Medium',
      assignedTo: userId,
    ...
    const updateSubtaskResponse = await makeRequest('PUT', `/brands/${brandId}/subtasks/${subtaskId}`, {
      description: 'Updated subtask description for comprehensive system testing'
    });
...
const testPhase7 = async () => {
  ...
  try {
    // 7.1 Create Comment with Markdown
    console.log('7.1 Testing comment creation with markdown...');
    const commentResponse = await makeRequest('POST', `/brands/${brandId}/tasks/${taskId}/comments`, {
      content: 'This is a **test comment** with *markdown* formatting and [link](https://example.com)!',
      mentions: [],
      links: []
    });
    commentId = commentResponse.data.data._id;
    recordTest('Create Comment with Markdown', commentResponse.data.success);
    
    // 7.2 Get Comments with Threading
    console.log('7.2 Testing get comments with threading...');
    const commentsResponse = await makeRequest('GET', `/brands/${brandId}/tasks/${taskId}/comments?thread=true&limit=20&offset=0`);
    recordTest('Get Comments with Threading', commentsResponse.data.success);
    ...
    // 7.5 Add Reply (Threading)
    console.log('7.5 Testing add reply (threading)...');
    ...
    recordTest('Add Reply (Threading)', replyResponse.data.success);
    ...
    // 7.7 Update Comment (Edit)
    console.log('7.7 Testing update comment (edit)...');
    ...
    recordTest('Update Comment (Edit)', updateCommentResponse.data.success);
...
const testDashboard = async () => {
  ...
  try {
    ...
    try {
      ...
      recordTest('Get Dashboard Data', false, 'Dashboard endpoint not implemented');
    }
...
const runCompleteSystemTest = async () => {
  console.log('🚀 STARTING COMPLETE SYSTEM TEST - FINAL VERSION');
  ...
  console.log('As per COMMENT_DOCUMENT.md specifications');
  console.log('='.repeat(80));
  ...
  try {
    ...
    // Print final results
    console.log('\n' + '='.repeat(80));
    console.log('📊 COMPLETE SYSTEM TEST RESULTS - FINAL');
    console.log('='.repeat(80));
    ...
    console.log('\n📋 IMPLEMENTED FEATURES (As per COMMENT_DOCUMENT.md):');
